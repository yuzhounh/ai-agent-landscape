...
| 维度     | 主要问题                         | 常见取值                                                            |
| -------- | -------------------------------- | ------------------------------------------------------------------- |
| 交互入口 | 用户在哪里指挥 Agent？           | IDE、终端、网页、桌面应用、GitHub、聊天工具                         |
| 执行位置 | 任务在哪里运行？                 | 本机、隔离沙箱、云端 VM、CI / GitHub Actions、移动设备              |
| 工作对象 | Agent 主要操作什么？             | 代码仓库、网页、桌面 GUI、本地文件、文档、表格、演示稿、第三方 SaaS |
| 自治方式 | Agent 能持续工作多久、如何交付？ | 同步协作、后台异步、定时任务、常驻服务、PR / 文件 / 报告交付        |
